...
export default async function PerfumeDetailPage(props: PerfumeProps) {
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-5 py-12 md:py-16">
      <Link
        href="/perfumes"
        className="inline-flex items-center text-sm font-bold text-[#ff4fa8] hover:text-[#22181f] transition-colors duration-200 mb-8"
      >
        ← Back to all perfumes
      </Link>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 bg-white/74 border border-white/85 rounded-[30px] p-6 md:p-10 shadow-[0_24px_60px_rgba(255,79,168,0.1)] backdrop-blur-xl">
        ...
        <div className="lg:col-span-6 flex flex-col gap-8">
          ...
          {(perfume.topNotes || perfume.heartNotes || perfume.baseNotes) && (
            <div className="p-6 md:p-8 rounded-2xl bg-white/40 border border-white/80">
              <h2 className="text-xl font-black text-[#22181f] mb-5 uppercase tracking-wide">
                Fragrance Notes
              </h2>
              <div className="flex flex-col gap-4">
                {perfume.topNotes && (
                  <div className="flex flex-col gap-1 border-b border-white/60 pb-3 last:border-0 last:pb-0">
                    <span className="text-xs font-black uppercase text-[#ff4fa8] tracking-widest">Top Notes</span>
                    <span className="text-[1.05rem] font-bold text-[#22181f]">{perfume.topNotes}</span>
                  </div>
                )}
                {perfume.heartNotes && (
                  <div className="flex flex-col gap-1 border-b border-white/60 pb-3 last:border-0 last:pb-0">
                    <span className="text-xs font-black uppercase text-[#ff4fa8] tracking-widest">Heart Notes</span>
                    <span className="text-[1.05rem] font-bold text-[#22181f]">{perfume.heartNotes}</span>
                  </div>
                )}
                {perfume.baseNotes && (
                  <div className="flex flex-col gap-1 border-b border-white/60 pb-3 last:border-0 last:pb-0">
                    <span className="text-xs font-black uppercase text-[#ff4fa8] tracking-widest">Base Notes</span>
                    <span className="text-[1.05rem] font-bold text-[#22181f]">{perfume.baseNotes}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="lg:col-span-6 flex flex-col justify-between gap-10">
          <div className="flex flex-col gap-5">
            <h1 className="text-4xl md:text-5xl font-black text-[#22181f] uppercase tracking-tight leading-tight">
              {perfume.title}
            </h1>

            {perfume.description && (
              <p className="text-base md:text-lg font-medium text-[#22181f]/80 leading-relaxed max-w-prose">
                {perfume.description}
              </p>
            )}
          </div>

          <div className="flex flex-col gap-5 p-6 rounded-2xl bg-[#ff4fa8]/5 border border-[#ff4fa8]/10">
            <h2 className="text-base md:text-lg font-black text-[#22181f] leading-snug">
                    ...
                    >
                      <span
                        ...
                        style={{
                          ...
                          border: link.store === 'amazon' ? '1px solid #e2e8f0' : 'none'
                        }}
                      >
                      ...
                      <div className="flex flex-col justify-center flex-1">
                        <span className="text-sm font-black text-[#22181f] uppercase tracking-wider">
                          {link.store.toUpperCase()}
                        </span>
                        {link.price && (
                          <span className="text-xs font-bold text-neutral-500 mt-0.5">
                            {link.price}
                          </span>
                        )}
                      </div>

                      <span className="text-xl font-extrabold text-[#22181f]/40 transition-transform duration-200 ease-in-out px-2 group-hover:translate-x-1 group-hover:text-[#ff4fa8]">
                        →
                      </span>
                    </a>
                  </li>

...
export const Card: React.FC<CardProps> = ({
  ...
  const baseClass =
    "w-full max-w-[900px] glass-panel rounded-card px-12 py-16 text-text-dark relative overflow-visible max-sm:px-5 max-sm:py-12 max-sm:rounded-card-sm max-sm:!transform-none";

  return (
    <div id={id} className={`${baseClass} ${className}`} style={inlineStyles}>
      {children}
    </div>
  );
};

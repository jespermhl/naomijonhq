...
export function NewsletterPromotion() {
  ...
  return (
    <div
      className={`fixed inset-0 z-[1000] flex items-center justify-center p-6 transition-all duration-400 ${isVisible ? "bg-black/30 pointer-events-auto backdrop-blur-[6px]" : "bg-transparent pointer-events-none"}`}
    ...
    >
      <div
        id="newsletter-promo"
        className={`glass-panel rounded-[30px] px-8 pt-9 pb-7 w-full max-w-[400px] relative flex flex-col items-center gap-3.5 text-center transition-all duration-400 ease-[cubic-bezier(0.34,1.56,0.64,1)] max-sm:px-5 max-sm:py-8 max-sm:pb-6 max-sm:rounded-[26px] max-sm:max-w-full ${isVisible ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-85 translate-y-4"}`}
      ...
      >
        <button
          onClick={handleDismiss}
          className="absolute top-3 right-3 bg-white/80 border border-white/90 rounded-full text-lg cursor-pointer text-brand-red p-1.5 flex items-center justify-center transition-transform duration-200 hover:scale-110 hover:rotate-90"
